...
@DATASETS.register_module()
class ModelNetDataset_Brackets(Dataset):
    def __init__(self,
                 split='train',
                 data_root='/media/chuanbo/DATA/data/托槽数据/托槽分割ShapeNet数据集',
                 class_names=None,
                 transform=None,
                 test_mode=False,
                 test_cfg=None,
    ...
    def get_data_list(self):
        assert isinstance(self.split, str)
        split_path = os.path.join(self.data_root, \
                                  f'train_test_split_{self.dataset_id}', \
                                  f'shuffled_{self.split}_file_list.json')
        # data_list = np.loadtxt(split_path, dtype="str")
        data_list = json.load(split_path)
        return data_list
    ...
    def get_data(self, idx):
        ...
        if self.cache_data:
            ...
            data_shape = '_'.join(self.data_list[data_idx].split('_')[0: -1])
            data_path = os.path.join(self.data_root, self.dataset_id, data_shape, self.data_list[data_idx] + '.txt')
            data = np.loadtxt(data_path, delimiter=',').astype(np.float32)
            coord, normal = data[:, 0:3], data[:, 3:6]
            category = np.array([self.class_names[data_shape]])
            if self.cache_data:
