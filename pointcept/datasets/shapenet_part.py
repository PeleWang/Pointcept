...
class ShapeNetPartDataset(Dataset):
    def __init__(
        ...
        self.token2category = {}
        with open(os.path.join(self.data_root, "synsetoffset2category_brackets.txt"), "r") as f:
            for line in f:
                ls = line.strip().split()
    ...
    def prepare_train_data(self, idx):
        ...
        data_dict = dict(coord=coord, normal=norm, segment=segment, cls_token=cls_token)
    ...
    def prepare_test_data(self, idx):
        ...
        data_dict = dict(coord=coord, normal=norm, cls_token=cls_token)
        data_dict = self.transform(data_dict)
        data_dict_list = []
...
@DATASETS.register_module()
class ShapeNetPartDataset_Brackets(Dataset):
    def __init__(
        self,
        split="train",
        data_root="/media/chuanbo/DATA2/data/托槽数据/托槽分割ShapeNet数据集",
        transform=None,
        test_mode=False,
        test_cfg=None,
        loop=1,
    ):
        super(ShapeNetPartDataset_Brackets, self).__init__()
        self.data_root = data_root
        self.split = split
        self.transform = Compose(transform)
        self.loop = (
    ...
    def prepare_test_data(self, idx):
        ...
        data_dict_list = []
        for aug in self.aug_transform:
            data_dict_post = self.post_transform(aug(deepcopy(data_dict)))
            if 'shift' in data_dict.keys():
                data_dict_post['shift'] = data_dict['shift']
            data_dict_list.append(data_dict_post)
        data_dict = dict(
            fragment_list=data_dict_list, segment=segment, name=self.get_data_name(idx)
        )
        return data_dict
